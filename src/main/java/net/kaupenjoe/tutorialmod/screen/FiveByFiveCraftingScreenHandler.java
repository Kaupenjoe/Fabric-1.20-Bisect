...
public class FiveByFiveCraftingScreenHandler extends AbstractRecipeScreenHandler<RecipeInputInventory> {
    private final RecipeInputInventory input;
    private final CraftingResultInventory result;
    private final ScreenHandlerContext context;
    private final PlayerEntity player;

    public FiveByFiveCraftingScreenHandler(int syncId, PlayerInventory playerInventory) {
        this(syncId, playerInventory, ScreenHandlerContext.EMPTY);
    }

    public FiveByFiveCraftingScreenHandler(int syncId, PlayerInventory playerInventory, ScreenHandlerContext context) {
        super(ModScreenHandlers.FIVE_BY_FIVE_CRAFTING, syncId);
        this.input = new CraftingInventory(this, 5, 5);
        this.result = new CraftingResultInventory();
        this.context = context;
        this.player = playerInventory.player;
        this.addSlot(new CraftingResultSlot(playerInventory.player, this.input, this.result, 0, 141, 48));
    ...
    protected static void updateResult(ScreenHandler handler, World world, PlayerEntity player, RecipeInputInventory craftingInventory, CraftingResultInventory resultInventory) {
        if (!world.isClient) {
            ...
            Optional<CraftingRecipe> optionalCrafting = world.getServer().getRecipeManager().getFirstMatch(RecipeType.CRAFTING, craftingInventory, world);
            Optional<FiveByFiveCraftingRecipe> optionalFiveByFive = world.getServer().getRecipeManager().getFirstMatch(FiveByFiveCraftingRecipe.Type.INSTANCE, craftingInventory, world);
